...
$claudeSettingsLocalPath = Join-Path $PSScriptRoot '.claude\settings.local.json'
if (Test-Path $claudeSettingsLocalPath) {
  $claudeSettingsLocal = Get-Content $claudeSettingsLocalPath -Raw
  Add-Result -Name 'claudeLocal:no-hardcoded-github-token' -Ok (-not ($claudeSettingsLocal -match 'gho_[A-Za-z0-9_]+|ghp_[A-Za-z0-9_]+|github_pat_[A-Za-z0-9_]+')) -Detail $claudeSettingsLocalPath
  Add-Result -Name 'claudeLocal:no-broad-user-read-scope' -Ok (-not ($claudeSettingsLocal -match 'Read\(//c/Users/<USERNAME>/\*\*\)|Read\(//c/Users/<USERNAME>/AppData/Local/GitHub/\*\*\)|Read\(//usr/\*\*\)|Read\(//usr/local/\*\*\)')) -Detail $claudeSettingsLocalPath
  Add-Result -Name 'claudeLocal:no-credential-manager-enumeration' -Ok (-not ($claudeSettingsLocal -match 'cmdkey /list')) -Detail $claudeSettingsLocalPath
}
