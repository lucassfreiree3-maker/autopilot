...
describe("postOasAutomation controller", () => {
  const originalEnv = process.env;
  let tmpDir = "";
  let dbPath = "";

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "oas-execute-controller-"));
  ...
  test("returns 404 when cluster/namespace is not registered", async () => {
    ...
    const { postOasAutomation } = await import(
      "../../controllers/oas-execute.controller"
    );

    const req = {
      params: { automation: "get_pods" },
      body: { cluster: "cluster-a", namespace: "ns-a" },
      headers: {},
      header: () => "",
    };
    const res = createMockResponse();

    await postOasAutomation(req as never, res as never);
