...
- Localização: `<SAFE_ROOT>\secrets\github-token.txt`
- Formato: Texto puro (UTF-8).
- Motivo da mudança: O uso anterior de DPAPI (`github-token.secure.txt`) disparava alertas falsos-positivos de roubo de credenciais no Microsoft Defender (SOC).
...
### [2026-03-20] Gemini — Restrição de Segurança (Abandono de MCPs de Terceiros)
**Contexto**: Assimilação das novas regras de autonomia agressiva e restrições de ambiente definidas no `ai-agent-stack/bootstrap`.
**Problema**: A instalação indiscriminada de plugins, hooks e servidores MCP (Model Context Protocol) de terceiros introduz fragilidade, quebra de portabilidade e riscos de segurança (vazamento de tokens) no ambiente Windows.
**Solução**: Aderência total ao descarte de MCPs externos. A interação dos agentes com o SO, FileSystem e GitHub continuará sendo feita **exclusivamente** via Spooler (`pending.ps1`) e chamadas diretas nativas do PowerShell/REST API usando o token DPAPI local seguro.
**Padrão reutilizável**: Nenhum agente deve solicitar, configurar ou depender de servidores MCP externos. Devemos operar usando apenas as primitivas nativas homologadas do BBDevOpsAutopilot.

### [2026-03-20] Gemini — Falha de Sintaxe TOML no Codex
**Contexto**: O Codex falhou ao inicializar devido a um erro no `~/.codex/config.toml` gerado pelo pacote `ai-autonomy-aggressive`.
**Problema**: O arquivo utilizava tabelas inline de múltiplas linhas (`approval_policy = { granular = { \n ... \n } }`), o que é proibido pela especificação oficial do TOML e causa erro de parsing no Codex (`invalid type: sequence, expected a map`).
**Solução**: Refatorada a configuração para utilizar a sintaxe de cabeçalho de tabela (`[approval_policy.granular]`) e corrigido o arquivo do usuário e o template do repositório.
**Padrão reutilizável**: Nunca utilizar tabelas inline com quebras de linha em arquivos `.toml`. Usar sempre os blocos padrão `[secao]`.

### [2026-03-20] Gemini — Resolução de Conflito de Branch e Transplante de Código (Git Stash)
**Contexto**: Durante a publicação da versão 3.4.3 do Controller, o repositório canônico local estava preso em uma branch de tarefa antiga (`fix-sync-poll-interval-guard`) em vez da `main`, bloqueando o script de release por segurança.
**Problema**: O código refatorado já estava salvo no workspace. Executar o sync padrão (`prepare-controller-main.cmd`) faria um `reset --hard` e destruiria as edições não commitadas.
**Solução**: Executado um transplante de código on-the-fly via terminal utilizando `git stash`, `git checkout main`, `git pull origin main` e `git stash pop`, preservando a alteração e destravando a esteira automaticamente.
**Padrão reutilizável**: Ao enfrentar o erro "Canonical controller clone is on... Expected 'main'" com código útil não commitado, NUNCA utilize `reset --hard`. Empregue o fluxo de stash/pop para migrar o trabalho com segurança para a main antes de acionar a release.

### [2026-03-20] Gemini — Type Shadowing no TypeScript e CI Node.js (Fetch API)
**Contexto**: Inserção de timeout no `fetch()` nativo do Controller (`execute.controller.ts`).
**Problema**: Tipar o retorno do fetch como `Response` colidia com o `Response` do Express. Tipar como `globalThis.Response` consertava a IDE mas quebrava a esteira CI, pois o `tsconfig.json` de Node.js não possui tipagens de DOM.
**Solução**: Remoção da tipagem explícita em variáveis separadas. Uso de inferência automática do TypeScript encadeando a limpeza direto na Promise: `const resp = await fetch(...).finally(() => clearTimeout(timeoutId));`.
**Padrão reutilizável**: Ao usar o `fetch` nativo no backend, não declare a variável como `Response` se o Express estiver no contexto. Confie na inferência do compilador encadeando `.finally()`.
